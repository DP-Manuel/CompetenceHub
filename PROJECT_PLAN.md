...
- Current status: green for the public static Website and yellow
  ...
  on the deployed clean Website checkpoint `9955e038fdbc` and
...
- Current critical path: close the narrowly scoped SB-54 Mindforge layout P0
  ...
  complete. The corrected local candidate passed 800/800 checks and the
...
4. **Mailbox smoke:** test one real contact-mail handoff with Janay when she is
   available; escalate routing failure without blocking the static Website.
5. **During absence:** allow only approved content fixes or incident recovery
   through the handover; no Portal-/Backend-Aktivierung.
6. **Messe check:** confirm the frozen public presentation before 17.10.
7. **After Messe / lower confidence:** resume native CAL-1 Staging UI and later
...
- Production now runs source `9955e038fdbc` with Christian Galvano and Manuela
  Rodriguez as the only real profiles plus six explicit demos. Public HTTP,
  sitemap, image and focused Edge checks passed; the six former personal routes
  return 404. The old production artifact is evidence only and not a valid
  Coach rollback.
